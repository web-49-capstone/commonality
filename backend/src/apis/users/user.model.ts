...
export async function findMatchingUsersByInterests(userId: string): Promise<PublicUser[]> {
  // Find users who share at least one interest with the given user, excluding the user themselves
  const rows = await sql`
    SELECT DISTINCT u.user_id, u.user_availability, u.user_bio, u.user_city, u.user_created, u.user_img_url, u.user_name, u.user_state
    FROM "user" u
    JOIN user_interest ui ON u.user_id = ui.user_interest_user_id
    WHERE ui.user_interest_interest_id IN (
      SELECT user_interest_interest_id FROM user_interest WHERE user_interest_user_id = ${userId}
    )
