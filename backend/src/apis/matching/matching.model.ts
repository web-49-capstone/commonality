...
export const MatchSchema = z.object({
    matchMakerId: z.uuidv7('please provide a valid uuid7 for matchId'),
    matchReceiverId: z.uuidv7('please provide a valid uuid7 for matchId'),
...
export async function insertMatch (match: Match): Promise<string> {
    ...
    await sql`INSERT INTO match(match_maker_id, match_receiver_id, match_accepted, match_created) VALUES (${matchMakerId}, ${matchReceiverId}, ${matchAccepted})`
    return 'Match added'
}
...
export async function selectMatchByMatchMakerId (matchMakerId: string ): Promise<Match|null> {
...
const rowList = await sql`SELECT match_maker_id, match_receiver_id, match_accepted, match_created FROM match WHERE match_receiver_id = ${matchReceiverId}`
const result = MatchSchema.array().max(1).parse(rowList)
return result[0] ?? null
}
