...
{
    const rowList = await sql`SELECT user_id, user_availability, user_bio, user_city, user_img_url, user_name, user_state FROM "user" INNER JOIN match ON (user_id = match_maker_id OR user_id = match_receiver_id) WHERE match_accepted = ${matchAccepted} AND (match_receiver_id = ${userId} OR match_maker_id = ${userId})`
