...
import { interestRoute } from './apis/users/interest.route.ts'
import { matchRoute } from './apis/users/match.route.ts'
import { messageRoute } from './apis/users/message.route.ts'
...
export class App {
  ...
  // private method that sets the port for the sever, to one from index.routes.ts, and external .env file or defaults to 3000
  ...
  private routes (): void {
    ...
    this.app.use(matchRoute.basePath, matchRoute.router)
  ...
  public listen (): void {
    ...
    console.log(`Express application built successfully on port ${this.app.get('port') as number}`)
  }
}
